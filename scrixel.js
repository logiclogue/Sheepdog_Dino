			...
			for (var a = 0; a < theCollisionGroup.boxes.length; a++) {
				for (var b = 0; b < mod.boxes.length; b++) {
					...
					if (boxA.x < boxB.x + boxB.w && boxA.x + boxA.w > boxB.x && boxA.y < boxB.y + boxB.h && boxA.y + boxA.h > boxB.y) {
						...
						// general
						if (mod.collisionFuncs[colGroupIndex] !== undefined) {
							mod.collisionFuncs[colGroupIndex].general(boxB);
						}
...
global.update = function() {
	...
	function draw() {
		global.ctx.clearRect(0, 0, global.actualWidth, global.actualHeight);
